...
from acta.rl import QLearningConfig, train_q_learning


def main() -> None:
    parser = argparse.ArgumentParser(description="Run tabular Q-learning with optional ActA priors.")
    parser.add_argument("games", type=Path, nargs="+")
    parser.add_argument("--prior", choices=["none", "acta-soft", "acta-hard"], default="none")
...
def summarize_traces(traces: tuple[Any, ...]) -> dict[str, Any]:
    ...
    noop_or_seen = sum(
        ...
        if "noop" in step.acta_reasons or "seen_state" in step.acta_reasons
    )
    blocked_actions = sum(
