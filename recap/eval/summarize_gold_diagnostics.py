...
def row_from_run(path: Path) -> dict[str, Any]:
    ...
    recovered = [step for step in gold_steps if step.get("recap_recovered_gold")]
    demotion_opportunities = [step for step in gold_steps if step.get("gold_rank_before") == 1]
    demoted = [step for step in gold_steps if step.get("recap_demoted_gold")]
    top1_bad = [step for step in steps if step.get("top1_structural_bad")]
    blocked_bad_top1 = [step for step in steps if step.get("recap_blocked_bad_top1")]
