...
from recap.agents import Agent, AgentContext, CandidateAction
from recap.controllers import PriorController, ControllerDecision
...
class AgentStepLog:
    ...
    recap_recovered_gold: bool = False
    recap_demoted_gold: bool = False
    top1_action: str | None = None
    top1_structural_reasons: tuple[str, ...] = ()
    top1_structural_bad: bool = False
    recap_blocked_bad_top1: bool = False
...
def run_episode(
    ...
    seed: int = 0,
    max_steps: int = 30,
    controller: PriorController | None = None,
...
def episode_metrics(steps: tuple[AgentStepLog, ...]) -> dict[str, float]:
    ...
    blocked_bad_top1 = sum(1 for step in steps if step.recap_blocked_bad_top1)
    ...
    return {
        ...
        "recap_recovered_gold": float(sum(1 for step in steps if step.recap_recovered_gold)),
        "recap_demoted_gold": float(sum(1 for step in steps if step.recap_demoted_gold)),
        ...
        "recap_blocked_bad_top1": float(blocked_bad_top1),
        "recap_blocked_bad_top1_rate": blocked_bad_top1 / top1_bad if top1_bad else 0.0,
