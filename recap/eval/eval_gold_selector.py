...
from recap.agents import Agent, AgentContext, CandidateAction, NoisyCandidateAgent
...
from recap.eval.agent_loop import first_policy_command, rank_action
from recap.eval.eval_agent import build_agent, build_controller_config
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate baseline top-1 vs ReCAP reranking on oracle TextWorld states."
    )
    ...
    for index, game in enumerate(args.games):
        ...
        if args.progress:
            selected = sum(record.recap_selected_gold for record in game_records)
            print(
                f"game={index + 1}/{len(args.games)} file={game} "
                f"states={len(game_records)} recap_selected_gold={selected}",
                flush=True,
            )
    ...
    if args.agent == "openai":
        summary["model"] = args.model or os.environ.get("RECAP_LLM_MODEL")
    summary["controller"] = "prior"
...
def evaluate_game(
    ...
    for step_index in range(max_steps):
        ...
        recap_action = decision.selected.action if decision.candidates else None
        ...
        selected_reasons = decision.reasons.get(recap_action, ()) if recap_action is not None else ()
        base_selected_gold = base_action == gold_action
        recap_selected_gold = recap_action == gold_action
...
def summarize(records: list[GoldSelectorRecord]) -> dict[str, Any]:
    ...
    recap_selected = [record for record in records if record.recap_selected_gold]
    recovery_opps = [
        record
        for record in records
        if record.gold_rank_before is not None and record.gold_rank_before != 1
    ]
    recovered = [record for record in records if record.recap_recovered_gold]
    demotion_opps = [record for record in records if record.gold_rank_before == 1]
    demoted = [record for record in records if record.recap_demoted_gold]
    top1_bad = [record for record in records if record.top1_structural_bad]
    blocked_bad = [record for record in records if record.recap_blocked_bad_top1]
    rank_before_values = [
    ...
    return {
        ...
        "recap_selected_gold": len(recap_selected),
        "recap_selected_gold_rate": rate(len(recap_selected), total),
        "recovery_opportunities": len(recovery_opps),
        "recap_recovered_gold": len(recovered),
        "recap_recovery_rate": rate(len(recovered), len(recovery_opps)),
        "demotion_opportunities": len(demotion_opps),
        "recap_demoted_gold": len(demoted),
        "recap_demotion_rate": rate(len(demoted), len(demotion_opps)),
        "top1_structural_bad": len(top1_bad),
        "top1_structural_bad_rate": rate(len(top1_bad), total),
        "recap_blocked_bad_top1": len(blocked_bad),
        "recap_blocked_bad_top1_rate": rate(len(blocked_bad), len(top1_bad)),
