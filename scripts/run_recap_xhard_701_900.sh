...
python -m recap.eval.bootstrap_recap_metrics \
  --ledger "analysis/${RUN}_ledger.jsonl" \
  --preferences "analysis/${RUN}_preferences.jsonl" \
  --iterations 1000 \
...
if [[ "${RUN_SIMPLE_BASELINES:-0}" == "1" ]]; then
  python -m recap.models.eval_random_reranker \
...
python -m recap.models.train_exact_memory_reranker \
  --train "analysis/${RUN}_splits_t30/train.jsonl" \
  --out "${MODEL_ROOT}/${RUN}_exact_memory_t30.json"

python -m recap.models.eval_exact_memory_reranker \
  --test "analysis/${RUN}_splits_t30/test.jsonl" \
...
if [[ "${RUN_SIMPLE_BASELINES:-0}" == "1" ]]; then
  for strategy in navigation-prior anti-static learned-verb-prior; do
    ...
    python -m recap.models.eval_heuristic_reranker \
      "${extra_train[@]}" \
      --test "analysis/${RUN}_splits_t30/test.jsonl" \
      --strategy "$strategy" \
...
python -m recap.models.train_sklearn_reranker \
  --train "analysis/${RUN}_splits_t30/train.jsonl" \
  --model-type mlp \
  --out "${MODEL_ROOT}/${RUN}_mlp_reranker_t30.joblib"

python -m recap.models.eval_sklearn_reranker \
  --test "analysis/${RUN}_splits_t30/test.jsonl" \
...
python -m recap.models.train_embedding_reranker \
  --train "analysis/${RUN}_splits_t30/train.jsonl" \
...
python -m recap.models.eval_embedding_reranker \
  --test "analysis/${RUN}_splits_t30/test.jsonl" \
...
python -m recap.eval.build_success_retention_dataset \
...
PY
  ...
  python -m recap.eval.eval_gold_demotion \
