...
This repo currently contains the first executable slice of ReCAP:
...
```bash
python -m recap.data.build_relation_dataset --out data/relation_records.jsonl
```
...
tw-make custom --world-size 3 --nb-objects 8 --quest-length 3 --seed 11 --output data/textworld_games/recap_seed11.z8 --force --silent
```

Probe action relations:

```bash
python -m recap.data.build_textworld_relation_dataset data/textworld_games/recap_seed11.z8 --out data/textworld_relation_records.jsonl --seed 0 --num-prefixes 8 --max-pairs-per-prefix 12
```

Probe a larger effectful-action slice:

```bash
python -m recap.data.build_textworld_relation_dataset data/textworld_games/recap_seed11.z8 --out data/textworld_relation_records_effectful.jsonl --seed 0 --num-prefixes 100 --max-depth 8 --max-pairs-per-prefix 10 --max-records 1000 --pair-filter both-effectful
```

Summarize relation records:

```bash
python -m recap.data.summarize_relations data/textworld_relation_records.jsonl
```

Audit signature diffs behind labels:

```bash
python -m recap.data.audit_signatures data/textworld_relation_records_effectful.jsonl --relation commute --value false
```

Build a small multi-seed TextWorld dataset:

```bash
python -m recap.data.build_textworld_batch --game-seeds 11 12 13 --num-prefixes 40 --max-records-per-game 400 --workers 3 --merged-out data/textworld_multi_seed.jsonl
```
...
```bash
python -m recap.data.relation_existence_table data/textworld_multi_seed.jsonl --out analysis/textworld_relation_existence.md
```

Evaluate normal-form rewriting on sampled traces:

```bash
python -m recap.eval.eval_normal_form data/textworld_multi_seed.jsonl --limit 100
```

Evaluate a random agent with and without ReCAP control:

```bash
python -m recap.eval.eval_agent data/textworld_games/recap_seed11.z8 data/textworld_games/recap_seed12.z8 --controller none --max-steps 20
python -m recap.eval.eval_agent data/textworld_games/recap_seed11.z8 data/textworld_games/recap_seed12.z8 --controller prior --max-steps 20 --max-candidates 8
python -m recap.eval.eval_agent data/textworld_games/recap_seed11.z8 data/textworld_games/recap_seed12.z8 --controller prior --fast-controller --max-steps 20 --max-candidates 8
```

Run the LLM-agent flow without external API calls:

```bash
python -m recap.eval.eval_agent data/textworld_games/recap_seed11.z8 --agent mock-llm --controller prior --fast-controller --max-candidates 5 --max-steps 12
```
...
```bash
cp .env.example .env
# edit .env with OPENAI_API_KEY and RECAP_LLM_MODEL
python -m recap.eval.eval_agent data/textworld_games/recap_seed11.z8 --agent openai --controller prior --fast-controller --max-candidates 5 --max-steps 12
```

Stress-test reranking under structurally noisy candidates:

```bash
python -m recap.eval.eval_agent data/textworld_games/recap_seed11.z8 --agent openai --candidate-noise frontload-existing-structural --controller prior --fast-controller --max-candidates 5 --max-steps 12
```

For longer API-backed runs, write per-episode output and continue past transient API failures:

```bash
python -m recap.eval.eval_agent data/textworld_games/recap_seed{11..15}.z8 \
  --agent openai \
  ...
  --episode-delay 5 \
  --continue-on-error \
  --episodes-out analysis/openai_noisy_existing_recap_fast_5games.episodes.jsonl \
  --out analysis/openai_noisy_existing_recap_fast_5games.json
```

Summarize gold-action recovery diagnostics when the environment exposes oracle policy commands:

```bash
python -m recap.eval.summarize_gold_diagnostics \
  analysis/textworld_hard_openai_base.json \
  analysis/textworld_hard_openai_recap_fast.json \
  --out analysis/textworld_hard_gold_diagnostics.md
```

Evaluate baseline top-1 vs ReCAP on the same oracle-path states:

```bash
python -m recap.eval.eval_gold_selector data/textworld_hard_games/*.z8 \
  --agent openai \
  --fast-controller \
  --max-candidates 5 \
  --max-steps 8 \
...
Run an ReCAP ablation without recent-repeat penalties:

```bash
python -m recap.eval.eval_agent data/textworld_hard_games/*.z8 \
  --agent openai \
  --controller prior \
  --fast-controller \
  ...
  --max-candidates 5 \
  --max-steps 20 \
  --out analysis/textworld_hard_openai_recap_no_repeat_penalty.json
```

Run a small tabular Q-learning comparison with ReCAP as an action prior:

```bash
python -m recap.eval.eval_rl data/textworld_games/recap_seed{11..15}.z8 \
  --prior prior-hard \
  --fast-controller \
  --episodes 30 \
  --max-steps 12 \
  ...
  --recent-repeat-penalty 0 \
  --out analysis/rl_textworld_easy_recap_hard_5games_3seeds.json

python -m recap.eval.compare_rl_runs \
  analysis/rl_textworld_easy_none_5games_3seeds.json \
  analysis/rl_textworld_easy_recap_soft_5games_3seeds.json \
  analysis/rl_textworld_easy_recap_hard_5games_3seeds.json \
...
```bash
python -m recap.eval.compile_traces analysis/textworld_hard_openai_base_5games.json \
  --progress \
  --out analysis/textworld_hard_openai_base_5games_trace_edits.json

python -m recap.eval.compare_trace_edits \
  analysis/textworld_hard_openai_base_5games_trace_edits.json \
  analysis/textworld_hard_openai_recap_fast_5games_trace_edits.json \
...
```bash
python -m recap.eval.eval_feedback_cycle data/textworld_games/recap_seed{11..15}.z8 \
  --agent openai \
  --feedback-mode safe \
  --max-candidates 5 \
  --max-steps 10 \
...
```bash
python -m recap.eval.build_action_preferences \
...
```bash
python -m recap.eval.eval_action_preferences \
  analysis/openai_xhard_feedback_cycle_safe_3games.json \
  --run-key first_run \
...
```bash
python -m recap.models.train_cross_encoder_reranker \
...
python -m recap.models.eval_cross_encoder_reranker \
